...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  const res = await fetch(`${API_URL}/syphons?${query}`)
    ...
    <Layout>
      {recipes.length === 0 ? (
        <h1 className='text-center font-bold text-3xl'>
          No syphon recipes found for "{router.query.term}"
        </h1>
      ) : (
        <h1 className='text-center font-bold text-3xl'>
          Found Syphon Recipes for "{router.query.term}"
        </h1>
      )}

      <a href='/syphon'>
        <BackButton text='Back to All Syphon Recipes' />
      </a>
